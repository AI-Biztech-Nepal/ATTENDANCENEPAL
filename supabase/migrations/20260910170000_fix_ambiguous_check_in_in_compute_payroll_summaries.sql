-- compute_payroll_summaries() could not run at all:
--
--   ERROR 42702: column reference "check_in" is ambiguous
--   It could refer to either a PL/pgSQL variable or a table column.
--
-- The function declares a local variable named check_in, and the
-- top-of-function delete (introduced in 20260909120000) tested an
-- unqualified "check_in is null" against payroll_summaries, which has a
-- check_in column too. PL/pgSQL refuses to guess, so every call has failed
-- since 20260909120000 was applied -- the nightly run and any manual
-- recompute alike. 20260910150000 carried the same statement over unchanged.
--
-- The delete's columns are now table-qualified. That was the only statement
-- where the clash could arise: the other uses of check_in are plain
-- variable assignments, INSERT column lists, excluded.check_in, and values
-- passed to calc_payroll_fields(), none of which put a same-named column in
-- scope.
--
-- Otherwise identical to 20260910150000's compute_payroll_summaries.
